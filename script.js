$(document).ready(function () {
    ...
    emailjs.init("ixiJ8ElRRDFSS9nD3");
...
document.addEventListener('visibilitychange',
    function () {
        if (document.visibilityState === "visible") {
            document.title = "Portfolio | Ravi Narayana";
...
var typed = new Typed(".typing-text", {
    strings: ["frontend development", "backend development", "Data Analyst"],
    loop: true,
    typeSpeed: 50,
    backSpeed: 25,
    backDelay: 500,
...
function showCertificates(certificates) {
  ...
  .forEach(cert => {
    certHTML += `
      <div class="box tilt">
        <img draggable="false" src="/assets/images/certificates/${cert.image}.png" alt="${cert.name}" />
        <div class="content">
          <div class="tag">
            <h3>${cert.name}</h3>
          </div>
  ...
  // Optional: Re-initialize effects
  VanillaTilt.init(document.querySelectorAll(".tilt"), {
    max: 15,
  });

  ScrollReveal().reveal('#certificates .box', { interval: 200 });
}
...
/* SCROLL INTERNSHIPS */
srtop.reveal('.internships .timeline', { delay: 400 });
srtop.reveal('.internships .timeline .container', { interval: 400 });
